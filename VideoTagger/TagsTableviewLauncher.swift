...
class TagsTableviewLauncher: NSObject {
    ...
    func handleShow(withDelay: TimeInterval ) {
        if let window = UIApplication.shared.keyWindow {
            ...
            tableView.frame = CGRect(x: -tvWidth, y: 98, width: tvWidth, height: 608)
            ...
            UIView.animate(withDuration: 0.5, delay: withDelay, usingSpringWithDamping: 1, initialSpringVelocity: 1, options: .curveEaseOut, animations: {
                ...
                self.tableView.frame = CGRect(x: 10, y: 98, width: self.tvWidth, height: 608)
                }, completion: nil)
        }
    }
    ...
    func handleDismiss() {
        ...
        UIView.animate(withDuration: 0.5, delay: 0, usingSpringWithDamping: 1, initialSpringVelocity: 1, options: .curveEaseOut, animations: {
            self.tableView.frame = CGRect(x: -self.tvWidth, y: 98, width: self.tvWidth, height: 608)
            }, completion: nil)
    }
